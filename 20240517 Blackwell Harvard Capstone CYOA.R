...
# Plot the components
plot(decomposition_result, xlab = "Year", ylab = "Sales", xaxt = "n")  # Remove default X-axis ticks

# Set custom X-axis ticks (whole years)
axis(1, at = seq(from = 2019, to = 2023), labels = seq(from = 2019, to = 2023))
...
# Fit the STL model to your time series data
stlf_model <- stlf(sales_ts, h = forecast_horizon)

# Generate forecasts using the STL model for testing data
stlf_forecast_testing <- forecast(stlf_model, h = forecast_horizon)

# Extract forecasted sales values from the STL forecast object
stlf_forecasted_sales <- stlf_forecast_testing$mean



# Plot actual sales and forecasted sales
plot(testing_data$Date, testing_data$Sales, type = "l", col = "blue", lwd = 2, ylim = c(0, max(max(testing_data$Sales), max(stlf_forecasted_sales))), xlab = "Date", ylab = "Sales", main = "STL Actual vs. Forecasted Sales")
...
summary_table <- data.frame(
  ...
  Linear = c(rmse_lm, mape_lm, forecast_bias_lm, fa_lm),
  ARIMA = c(rmse_arima, mape_arima, forecast_bias_arima, fa_arima),
  STL = c(rmse_stlf, mape_stlf, forecast_bias_stlf, fa_stlf))
